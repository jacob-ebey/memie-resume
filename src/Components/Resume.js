    ...
    if(this.props.data){
      ...
      var research = this.props.data.research.map(function(research){
        return <div key={research.company}><h3>{research.company}</h3>
            <p className="info">{research.title}<span>&bull;</span>{research.lab}<span>&bull;</span><em className="date">{research.years}</em></p>
      ...
      var skills = this.props.data.skills.map(function(skills){
        ...
        return <li key={skills.name}><span style={{width:skills.level}}className={className}></span><span className="bar-label">{skills.name}</span></li>
      })
    }
